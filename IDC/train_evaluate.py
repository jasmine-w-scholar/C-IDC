...
#python IDC/train_evaluate.py --cfg IDC/SYN-BIO-6.YAML
...
class BaseModule(LightningModule):
    def __init__(self, cfg):
       ...
       # self.train_dataset = getattr(dataset, cfg.dataset).setup(cfg)
        ...
        ###########################################
        #初始化数据集
        self.train_dataset = getattr(dataset, cfg.dataset).setup(
            filepath_samples=cfg.filepath_samples,
            filepath_labels=cfg.filepath_labels if 'filepath_labels' in cfg else None,
            num_clusters=cfg.n_clusters if 'n_clusters' in cfg else None
        )
        ###########################################
