...
class TotalCodingRateWithProjection(torch.nn.Module):
    ...
    def compute_discrimn_loss(self, W):
        p, m = W.shape  # [d, B]
        scalar = p / (m * self.eps)
        # ✅ 内存优化：Sylvester 行列式定理
        #   det(I_p + s·W·W^T) = det(I_m + s·W^T·W)
        #   当 p >> m 时（7129 >> 6），把 7129×7129 的 logdet 降到 6×6，
        #   显存从 ~600MB 降到 <1MB，速度从 O(p³) 降到 O(m³)，数值也更稳定。
        M = torch.eye(m, device=W.device) + scalar * (W.T @ W)
