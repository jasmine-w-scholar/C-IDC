# ============================================================
# 使用方式：
#   python sefs_opt_bayes.py  --cfg cfg/test.yaml  --n_calls 40  --batch_size 4  --n_workers 6  --n_seeds 10  --early_stop --patience 30  --min_delta 0.001
# 单独搜索acc
# 依赖：
...
def evaluate_single_params(
    ...
    try:
        ...
        cfg.correlation_threshold = float(correlation_threshold)
...
class SEFSBayesianOptimizer:
    ...
    SEARCH_SPACE = [
        ...
        Real(0.1, 0.9, name='correlation_threshold'),
        # ── 训练周期 ──
        Integer(1000,  2000, name='epochs'),
        Integer(20,   50,   name='ae_non_gated_epochs'),
        Integer(250,  400,  name='ae_pretrain_epochs'),
        Integer(330,  550,  name='start_global_gates_training_on_epoch'),

        # ── 网络架构 ──
        Integer(96,   256,  name='gates_hidden_dim'),

        # ── 学习率（log 均匀分布更合理）──
        Real(2e-3, 8e-3, 'log-uniform', name='lr_pretrain'),
        Real(2e-3, 1e-2, 'log-uniform', name='lr_clustering'),
        Real(1e-4, 5e-4, 'log-uniform', name='lr_aux_classifier'),
        ...
        # 需要更强的 L0 正则化来压制冗余特征
        Real(0.4,  1.2,  name='local_gates_lambda'),
        Real(1e-5, 1e-3, name='global_gates_lambda'),
        Real(0.01, 0.05,  name='gtcr_lambda'),

        # ── 聚类参数 ──
        Real(25.0, 80.0, name='tau'),
        Real(0.05,  0.15,  name='eps'),
        Real(0.3,   0.6,  name='mask_percentage'),
